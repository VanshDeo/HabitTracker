...
export default function NewHabitPage() {
  ...
  const { txState, createHabit, refresh } = useHabits(publicKey);
    ...
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mb-8">
        ...
        <p className="mt-1.5 text-sm text-muted-foreground leading-relaxed">
          Build consistency and earn HABIT tokens for every streak milestone — recorded on Stellar Soroban.
        </p>
      </div>

      {/* Reward table */}
      <div className="mb-8 rounded-2xl border border-border bg-card p-5 shadow-sm">
        <h2 className="mb-3 text-sm font-semibold text-foreground">Token Reward Tiers</h2>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border">
              <th className="pb-2 text-left font-semibold text-muted-foreground">Streak</th>
              <th className="pb-2 text-center font-semibold text-muted-foreground">Tokens / check-in</th>
              <th className="pb-2 text-right font-semibold text-muted-foreground">Badge</th>
            </tr>
          </thead>
          <tbody>
